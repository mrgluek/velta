...
- **Windows desktop** вЂ” a Tauri 2 app that bundles `deltachat-rpc-server.exe` as a sidecar.
- **Android mobile** вЂ” the same Tauri 2 app, but the Delta Chat core runs in-process inside the APK. A foreground service keeps sync (and notifications) running after the app is backgrounded.
- **Browser/PWA** вЂ” the same frontend can be served statically and connects to a local `velta-core-service` over loopback WebSocket/HTTP, or falls back to a mock core for demo purposes. The PWA's target deployment is a **remote core service over WSS/TLS** вЂ” the loopback bridge remains the local/dev path. If the loopback connection drops mid-session (service restart), the app reconnects automatically вЂ” backoff up to 15 s вЂ” and refreshes the chat list; no reload needed.
...
- **Invite QR / pasted code** вЂ” the ticket carries a one-time pairing token;
  presenting it is the out-of-band proof and pairing completes immediately.
- **Nearby tap** вЂ” sends a pairing request that the other device must
...
"Enter local chatвЂ¦" вЂ” it enables local chat and skips relay setup; enabling
...
arrive only from pairing. The hub вЂ” device identity, invite QR, add contact,
nearby-device pairing вЂ” is a collapsible card pinned above the chat list
...
the item) and the audio-call button is hidden there вЂ” the P2P engine carries
no call signaling.
...
message failed with a Retry button (re-sends the stored copy from byte zero вЂ”
...
the composer (`#lc-queue-chip` в†’ popup with per-item send-now/remove); the
...
px-sized boxes вЂ” broken layouts on devices set to the largest scaling
...
call signaling вЂ” the offer and answer SDP ride in encrypted messages вЂ” and
...
Velta renders it as a compact contact card вЂ” avatar, display name and email
address вЂ” styled like the invite cards. Tapping the card imports the contact
...
codes вЂ” SecureJoin QRs exist only for the own profile and for groups вЂ” so the
...
The drawer's **Add a second deviceвЂ¦** moves a profile between devices over the
...
- **New device** taps **Receive a profile on this deviceвЂ¦**, scans or pastes
  the code (`DCBACKUP2:вЂ¦`, camera scan everywhere вЂ” native `BarcodeDetector`
  where the platform provides it, a built-in decoder fallback where it
  doesn't), and a fresh account is created and filled
  from the transfer вЂ” progress is reported live, and the other device stays
  signed in.

The **Welcome to Velta** splash (full-screen, with the same choices: create a
new profile on a relay вЂ” type the address or scan its QR вЂ” **add as second
...
One profile can be reachable on **several chatmail relays** at once вЂ” what
...
**Relays of this profileвЂ¦** в†’ **"Use for sending"** on any relay (messages
...
- **Add relay** вЂ” paste a `dcaccount:`/`dclogin:` invite code or just the relay's domain (`nine.testrun.org`); the core
  configures it as a second transport (progress modal during setup).
- **Use for sending** вЂ” makes that relay the primary (sending) transport.
- **Remove relay** вЂ” immediate removal (core 2.60.0): the relay stops being
  ...
  last relay cannot be removed вЂ” the core re-elects a sending relay if
  needed.
...
list with one row per relay вЂ” status and quota usage вЂ” fed by the core's
connectivity page.
...
|---|---|---|
| Android | `https://i.delta.chat/#FINGERPRINT&v=3&вЂ¦` (or a registered mirror domain, e.g. `https://i.gluek.info/#вЂ¦`) | Intercepted by the Android intent filters and processed in-app. |
...
| Android | `dclogin:вЂ¦` | Chooser: add the (existing) relay account to the current profile. |
| Android | `DCBACKUP2:вЂ¦` (second-device code) | Opens Velta's "Receive a profile" transfer flow directly. |
...
| Contact verification | `OPENPGP4FPR:вЂ¦` | Can be processed as a SecureJoin/verification QR. |

### Invite link mirrors and invite cards

An invite link's payload lives in the URL fragment (`#FINGERPRINT&v=3&вЂ¦`) and is never
sent to a server вЂ” the host is decorative. Velta therefore accepts any **registered
...
- **Registry** вЂ” built-in hosts (`i.delta.chat`, `i.gluek.info`) plus user-added ones,
  managed in the drawer under *Settings в†’ Invite link domains* (stored in
  `localStorage["velta-invite-hosts"]`, logic in `app/js/invites.js`).
- **OS-level interception is compile-time** вЂ” Android intent filters live in
...
- **Invite cards** вЂ” an invite link inside a message renders as a card instead of a raw
...
Windows does **not** allow a normal desktop app to intercept a specific `https://` host like `i.delta.chat` вЂ” that power belongs to the default browser. So on Windows Velta registers a custom URI scheme (`velta://`) through `tauri-plugin-deep-link`. The first time the app runs it writes the registry entry for the scheme, after which the OS will launch Velta for any `velta://вЂ¦` link.
...
- **Android** вЂ” `velta-app/src-tauri/gen/android/app/src/main/AndroidManifest.xml` declares `VIEW` intent filters for `https://i.delta.chat` and mirror domains (`i.gluek.info`; one filter each вЂ” add more there and rebuild to extend OS-level interception). The Rust layer emits the URL to the frontend as a `deeplink` event.
- **Windows/Linux** вЂ” `tauri-plugin-deep-link` registers the `velta://` scheme. `tauri-plugin-single-instance` (with the `deep-link` feature) forwards second-instance launches to the running window. The plugin emits a `deep-link://new-url` event that the frontend listens to.
- **Common frontend handling** вЂ” `app/js/app.js` has `extractJoinLink()`, `extractInviteLink()`, and `extractVeltaLink()`. They normalise every supported format and route it to either the SecureJoin flow (`joinFromInvite`) or the account-setup flow (`addAccountFromInvite`). Link recognition, mirroring, and the invite-card rendering live in `app/js/invites.js`.

### Testing locally

- **Android** вЂ” tap an `https://i.delta.chat/#вЂ¦` link from any app. The system should offer to open it with Velta.
- **Windows** вЂ” after installing and running Velta once, open a `velta://invite?url=вЂ¦` link from a browser address bar or a local HTML file. The app should open and show the join progress modal.

### Limitations

- Windows cannot intercept the official `https://i.delta.chat/#вЂ¦` links directly. To make those links open Velta automatically on Windows, a browser extension that rewrites them to `velta://` URLs would be required.
...
- `app/js/chat-view.js` вЂ” attachment menu, native file picker (`plugin:dialog|open`), media rendering and the download button.
- `app/js/rpc-core.js` вЂ” `sendMessage()`, `getMessage()` and `downloadFullMessage()` wrappers around the core JSON-RPC methods.
- `app/css/main.css` вЂ” styles for `.msg-image`, `.msg-video`, `.msg-audio` and `.msg-file`.
...
(`_rowCache` in `app/js/chat-view.js`) survives chat switches вЂ” reopening a
...
- **Windows / desktop** вЂ” file pickers return real filesystem paths and everything works end-to-end.
- **Android** вЂ” the Tauri dialog may return a `content://` URI that the Delta Chat core cannot read directly. Velta copies picked files into the appвЂ™s local data directory using `tauri-plugin-fs` before passing an absolute path to `send_msg`.
...
| `[label](https://вЂ¦)` | clickable link (bare URLs linkify too) |
| `- item` / `* item` / `+ item` | bulleted list |
| `1. item` / `1) item` | numbered list (a start value like `3.` is honored) |

Messages whose original differs from the simplified bubble вЂ” the core's mail
simplifier cut a footer/quote (the text ends in `[...]`), or an HTML mail was
flattened to text вЂ” render a **Show Full MessageвЂ¦** button (the official
...
CSP applies inside the viewer) вЂ” nothing about you leaks to mail-embedded
trackers.

Everything is HTML-escaped before any tag is produced and only `http(s)` targets become links, so message content can never inject markup. Emphasis markers are word-boundary guarded (`2*3*4` and `snake_case_name` stay literal). Invite links (`i.delta.chat` and registered mirror domains) render as invite cards instead of links вЂ” see [Deep links](#deep-links). Invites carrying a `b=` parameter are broadcast channels and render as *"Subscribe to ChannelName"* with a Subscribe confirmation instead of the group wording.

Hovering a message on desktop shows a small **Reply** pill at the bubble's top-right corner вЂ” one click sets the reply (same pipeline as the context menu's Reply) and focuses the composer. The pill is hidden on touch devices and during message selection.

Note: other Delta Chat clients render only the core's markdown subset (bold, italic, strikethrough, code). Underline and lists are Velta-side rendering niceties вЂ” other clients show those markers literally.
...
message" bar, вњ• cancels); Enter applies. Editing updates the message in place
...
restricted to what the core allows: your own, plain-text, non-empty messages вЂ”
...
folder (the same mechanism as Delta Chat desktop вЂ” collections are folders
...
- **Android** вЂ” real MessagingStyle conversations: group name as the title,
...
- **Windows** вЂ” a toast with three text lines (chat name / sender / text)
...
(tauri-plugin-updater, minisign), runs it and relaunches the app вЂ” no browser,
...
keeps the banner's Download APK flow (system installer takes over) вЂ” see
...
Deleting a message (long-press / right-click в†’ Delete) opens the same dialog as
...
chat must not be Saved Messages. Deleting for everyone is a request вЂ” members
...
Unencrypted 1:1 chats вЂ” classic-email contacts that chatmail relays cannot
encrypt to вЂ” are marked with a small open-shackle lock in the chat list and
chat header instead.
...
Messages containing a `.xdc` mini-app render as an app card вЂ” the app's
...
themed to match the shell (dark canvas, dark scrollbars) вЂ” an app can still
...
channels are not вЂ” apps that rely on them degrade gracefully to status
updates.
...
the bubble вЂ” tap one and the command lands in the composer, ready to send.
...
2 rects, 3 squares вЂ” one cell per fingerprint group), with a fingerprint glyph
...
black/white), and everything is drawn as pure SVG in `app/js/avatar.js` вЂ”
...
initials вЂ” the identity matrix is a per-contact feature.
...
verified key from a **secure-join QR handshake** вЂ” scanned directly, or
...
an RPC each. Contact verification is a 1:1 concept вЂ” group protection status
is a separate core feature the JSON-RPC chat types don't expose yet.

**Brutal theme** (Settings в†’ Theme в†’ Brutal). A third, always-dark theme
built on neobrutalism principles and translated to Velta's token set вЂ” no
...
works the same way вЂ” it opens a live chat-name search in place of the list
and flips to a cross to close. The core keeps no call log вЂ” calls exist only
as call messages вЂ” so the Calls view lists calls ended on this device,
recorded locally and capped at 30. Every view button can be hidden in
Settings в†’ Bottom bar buttons; the Menu and + buttons are always visible,
...
**History loading strip.** While a chat's history loads вЂ” opening it or
paging up to older messages вЂ” a thin blue gradient sweeps left-to-right just
...
The palette avoids pure black and pure white everywhere вЂ” whites live in the `#f2f2f5`/`#f4f4f4` family and blacks in the `#0b0b10`вЂ“`#1c1c26` family (the avatar identity tiles already followed this rule). Text and surface pairs are held to **WCAG AA (в‰Ґ 4.5:1 contrast)**, measured with the WCAG relative-luminance formula after compositing any translucent layers:
...
Velta ships **zero analytics, telemetry, or crash reporting** вЂ” verified
...
missing the WebView2 runtime fetches it via Tauri's install bootstrap вЂ” that
...
(links inside are inert by design вЂ” the sandbox blocks popups and top-level
...
strictly same-origin вЂ” no inline scripts, no `unsafe-eval`; the pre-paint
...
rollback rules. What each core release gives Velta вЂ” the new features rated
by value with their integration notes вЂ” lives in
...
```
.
в”њв”Ђв”Ђ app/                        # Velta web frontend (vanilla JS, no build)
в”њв”Ђв”Ђ velta-app/              # Tauri 2 wrapper for Windows + Android
в”‚   в””в”Ђв”Ђ src-tauri/
в”‚       в”њв”Ђв”Ђ Cargo.toml          # Rust crate + deltachat-jsonrpc dependency
в”‚       в”њв”Ђв”Ђ tauri.conf.json     # shared Tauri config
в”‚       в”њв”Ђв”Ђ tauri.android.conf.json
в”‚       в”њв”Ђв”Ђ tauri.ios.conf.json
в”‚       в”њв”Ђв”Ђ gen/android/        # generated Android project
в”‚       в””в”Ђв”Ђ src/
в”‚           в”њв”Ђв”Ђ lib.rs          # sidecar + Android in-process core glue
в”‚           в”њв”Ђв”Ђ p2p.rs          # local chat engine (iroh QUIC pairing + 1:1 chat)
в”‚           в”њв”Ђв”Ђ bin/p2p-hub.rs  # headless terminal hub (debug helper)
в”‚           в””в”Ђв”Ђ main.rs         # Tauri entry point
в”њв”Ђв”Ђ velta-core-service/         # Android background-service (JNI + WS bridge) APK
в”њв”Ђв”Ђ deltachat-backend/          # Prebuilt deltachat-rpc-server binaries
в”‚   в”њв”Ђв”Ђ windows-x86_64/
в”‚   в””в”Ђв”Ђ android-arm64/
в”њв”Ђв”Ђ tools/                      # icon generation, WSL APK build/sign helpers,
в”‚                               # serve-dev.py (no-cache static server for app/)
в”њв”Ђв”Ђ signing/                    # local signing keystore (untracked)
в””в”Ђв”Ђ core/                       # Vendored Delta Chat core Rust workspace
```
...
The runtime has exactly **two vendored JavaScript libraries** вЂ” everything else
...
|---|---|---|
| [Elena](https://github.com/arielsalminen/elena) (`@elenajs/core` v1.0.1) | Tiny progressive web-components library вЂ” powers `<velta-avatar>`, `<velta-chat-item>`, `<velta-chat-head>`, `<velta-video>` | [arielsalminen/elena](https://github.com/arielsalminen/elena) |
...
`velta-app/src-tauri/tauri.conf.json` вЂ” the single source of truth:

- `Velta-<version>-<abi>.apk` вЂ” signed Android APK (`build-android.yml`)
- `Velta_<version>_x64-setup.exe` вЂ” NSIS Windows installer (`build-windows.yml`),
  plus its `Velta_<version>_x64-setup.exe.sig` updater signature
- `latest.json` вЂ” Windows self-update manifest (installer URL + signature, fed
  ...
  same release вЂ” a manifest visible before its installer bricks that update
  cycle.
- `version.txt` вЂ” just the version. Apps fetch
...
**Release changelog**: the release body is generated automatically вЂ” every
...
`ntfy.gluek.info/velta_changelog` вЂ” title `Velta: version bumped to
...
cancel each other (that is what silently killed the 1.4.6вЂ“1.4.8
...
falls back to an ephemeral key and warns вЂ” those APKs must not be published.
